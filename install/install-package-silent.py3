...
rConfigPath = "/home/odiniptv/config"
...
def prepare(rType="MAIN"):
    try:
        subprocess.check_output("getent passwd odiniptv > /dev/null".split())
    except:
      printc("Creating user")
      os.system("adduser --system --shell /bin/false --group --disabled-login odiniptv >/dev/null 2>&1")
      os.system("adduser --system --shell /bin/false odiniptv >/dev/null 2>&1")
      if not os.path.exists("/home/odiniptv"): os.mkdir("/home/odiniptv")
    return True
...
def mysql(rUsername, rPassword):
    ...
    os.system("mysql -u root%s xtream_iptvpro < /home/odiniptv/database.sql 2> /dev/null" % rExtra)
    ...
    os.system("rm -f /home/odiniptv/database.sql >/dev/null 2>&1")
    return True
...
def configure():
    ...
    if not "/home/odiniptv/" in open("/etc/fstab").read():
        rFile = open("/etc/fstab", "a")
        rFile.write(
            "tmpfs /home/odiniptv/streams tmpfs defaults,noatime,nosuid,nodev,noexec,mode=1777,size=90% 0 0\ntmpfs /home/odiniptv/tmp tmpfs defaults,noatime,nosuid,nodev,noexec,mode=1777,size=2G 0 0")
        rFile.close()
    if not "odiniptv" in open("/etc/sudoers").read():
        os.system('echo "odiniptv ALL=(root) NOPASSWD: /sbin/iptables, /usr/bin/chattr" >> /etc/sudoers')
    if not os.path.exists("/etc/init.d/odiniptv"):
        rStart = open("/etc/init.d/odiniptv", "w")
        rStart.write(
            "#!/bin/bash\n### BEGIN INIT INFO\n# Provides:          odiniptv\n# Required-Start:    $all\n# Required-Stop:\n# Default-Start:     2 3 4 5\n# Default-Stop:\n# Short-Description: Run /etc/init.d/odiniptv if it exist\n### END INIT INFO\nsleep 1\n/home/odiniptv/start_services.sh > /dev/null")
        rStart.close()
        os.system("chmod +x /etc/init.d/odiniptv")
        os.system("update-rc.d odiniptv defaults >/dev/null 2>&1")
        os.system("update-rc.d odiniptv enable >/dev/null 2>&1")
    try:
        os.remove("/usr/bin/ffmpeg")
    except:
        pass
    if not os.path.exists("/home/odiniptv/tv_archive"): os.mkdir(
        "/home/odiniptv/tv_archive/")
    os.system("ln -s /home/odiniptv/bin/ffmpeg /usr/bin/")
    os.system("chown odiniptv:odiniptv -R /home/odiniptv > /dev/null")
    os.system("chmod -R 0777 /home/odiniptv > /dev/null")
    os.system("chmod +x /home/odiniptv/start_services.sh > /dev/null")
    os.system("chattr -i /home/odiniptv/GeoLite2.mmdb > /dev/null")
    ...
    os.system("cp /usr/local/bin/youtube-dl /home/odiniptv/bin/ > /dev/null")
    os.system("sudo chmod a+rx /home/odiniptv/bin/youtube-dl > /dev/null")


def start():
    printc("Restarting ODIN")
    os.system("chattr +i /home/odiniptv/GeoLite2.mmdb 2>/dev/null")
    os.system("chmod 644 /home/odiniptv/php/VaiIb8.pid 2>/dev/null")
    os.system("chmod 644 /home/odiniptv/php/JdlJXm.pid 2>/dev/null")
    os.system("chmod 644 /home/odiniptv/php/CWcfSP.pid 2>/dev/null")
    os.system("chmod +x /home/odiniptv/nginx/sbin/nginx 2>/dev/null")
    os.system("chmod +x /home/odiniptv/nginx_rtmp/sbin/nginx_rtmp 2>/dev/null")
